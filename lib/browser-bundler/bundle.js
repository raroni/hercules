(function() {
  ...
  module.exports = Bundle = (function() {
    ...
    Bundle.prototype.toString = function() {
      var client_js, client_path, key, replaces, value;
      client_path = path.join(__dirname, 'client.js');
      client_js = fs.readFileSync(client_path, 'utf-8');
      replaces = {
        "'[[[source_files]]]'": this.sourceFilesAsJSON(),
        "'[[[package_files]]]'": this.packageFilesAsJSON()
      };
      for (key in replaces) {
        value = replaces[key];
        client_js = client_js.replace(key, value);
      }
      return client_js;
    };
